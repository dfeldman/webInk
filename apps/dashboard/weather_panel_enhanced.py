...
class WeatherPanelEnhanced(BasePanel):
    ...
    def get_html(self, size: str) -> str:
        ...
        # Build additional details based on size
        details_html = f"""
        ...
        # Add extended metrics for larger panels
        if size in ['2x2', '1x2']:
        ...
        forecast_limit = {
            ...
            '1x2': 5,
            '2x1': 3,
            '1x1': 0  # No forecast in small size
        }.get(size, 3)
        
        if forecast_limit > 0 and self.data['forecast']:
